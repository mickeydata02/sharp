...
def process_packet_command(message):
    ...
    try:
        ...
        bot.send_message(chat_id, f"*Packet size set to {packet_size} for sharp.*", parse_mode='Markdown')
...
def process_thread_command(message):
    ...
    try:
        ...
        bot.send_message(chat_id, f"*Thread count set to {thread_count} for sharp.*", parse_mode='Markdown')
...
async def run_attack_command_on_codespace(target_ip, target_port, duration, chat_id, user_id):
    try:
        ...
        # Construct command for sharp binary with thread count and packet size
        command = f"./sharp {target_ip} {target_port} {duration} {packet_size} {thread_count}"
...
def send_owner_info(message):
    owner_message = "This Bot Has Been Developed By ᚛ @GOKU_VAMPIRE ᚜"  
...
def send_welcome(message):
    ...
    if is_super_admin(user_id):
        welcome_message = (
            f"Welcome, Super Admin! Developed By ᚛ @GOKU_VAMPIRE ᚜\n\n"
            f"Admin Commands:\n"
        ...
        balance = get_admin_balance(user_id)
        welcome_message = (
            f"Welcome, Admin! Developed By ᚛ @GOKU_VAMPIRE ᚜\n\n"
...
def attack_button_handler(message):
    ...
    if not found_user:
        bot.send_message(chat_id, "*𝐘𝐨𝐮 𝐚𝐫𝐞 𝐧𝐨𝐭 𝐫𝐞𝐠𝐢𝐬𝐭𝐞𝐫𝐞𝐝. 𝐏𝐥𝐞𝐚𝐬𝐞 𝐫𝐞𝐝𝐞𝐞𝐦 𝐀 𝐤𝐞𝐲 𝐓𝐨 𝐎𝐰𝐧𝐞𝐫:- ᚛ @GOKU_VAMPIRE ᚜*", parse_mode='Markdown')
        return

    valid_until = datetime.fromisoformat(found_user['valid_until'])
    if datetime.now() > valid_until:
        bot.send_message(chat_id, "*𝐘𝐨𝐮𝐫 𝐤𝐞𝐲 𝐡𝐚𝐬 𝐞𝐱𝐩𝐢𝐫𝐞𝐝. 𝐏𝐥𝐞𝐚𝐬𝐞 𝐫𝐞𝐝𝐞𝐞𝐦 𝐀 𝐤𝐞𝐲 𝐓𝐨 𝐎𝐰𝐧𝐞𝐫:- ᚛ @GOKU_VAMPIRE ᚜.*", parse_mode='Markdown')
        return
    ...
    if is_super_admin(user_id):
        ...
        if datetime.now() > datetime.fromisoformat(found_user['valid_until']):
            account_info = (
                ...
                "Contact ᚛ @GOKU_VAMPIRE ᚜ for assistance."
            )
        else:
            account_info = (
        ...
        account_info = "𝐏𝐥𝐞𝐚𝐬𝐞 𝐫𝐞𝐝𝐞𝐞𝐦 𝐀 𝐤𝐞𝐲 𝐓𝐨 𝐎𝐰𝐧𝐞𝐫:- ᚛ @GOKU_VAMPIRE
